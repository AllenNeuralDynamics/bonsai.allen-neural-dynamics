...
{
    /// <summary>
    /// Represents an operator that sends a command to a <see cref="ZaberDevice"/> expecting a single <see cref="Response"/>.
    /// </summary>
    [Description("Returns the result of a command.")]
    public class GenericCommand : Combinator<string, Response>
    {
        /// <summary>
        ...
        /// <summary>
        /// Sends a command to the device expecting a single reply.
        /// </summary>
        /// <returns>
            ...
            return Observable.Using(
                ...
                async (connection, cancellationToken) => source.Select(value =>
                    Observable.FromAsync( async token =>
                        await connection.Device.GenericCommand(Device, Axis, value)
                    )).Concat());
        }
    }
}
